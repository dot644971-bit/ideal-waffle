...
export default function SeriesPage() {
  ...
  /* ── FIX: Doğrudan URL yüklemesinde (/series?slug=...) slug'ı URL'den oku ── */
  ...
  /* Store'dan gelen slug öncelikli (uygulama içi nav), yoksa URL'den al */
  ...
  const sliderScrollBy = (direction: 'prev' | 'next') => {
    ...
    if (!track) return;
    track.scrollBy({ left: direction === 'prev' ? -540 : 540, behavior: 'smooth' });
  };
  ...
  const genres = series
    ? series.genre.split(',').map((g) => g.trim()).filter(Boolean).slice(0, 3)
    : [];
  ...
  const seriesEpCount = series?.episodes ? parseInt(series.episodes, 10) : undefined;
  const currentYear = new Date().getFullYear();

  /* ── Slug henüz yüklenmedi (client hydration bekleniyor) ── */
  if (!slug) {
    return (
      <>
    ...
    return (
      <div
        style={{
          ...
          color: '#e5e5e5', fontFamily: "'Outfit', sans-serif", background: '#0a0a0a',
        }}
      >
        ...
        <button
          ...
          style={{
            background: '#fff', color: '#000', fontFamily: "'Outfit', sans-serif",
            fontSize: '0.95rem', fontWeight: 700, padding: '12px 26px',
  ...
  return (
    <>
      ...
      <div className="series-hero">
        <div className="hero-bg" style={{ backgroundImage: `url('${posterUrl(series.name)}')` }} />
        <div className="hero-vignette" />
        <div className="hero-side-vignette" />
        <div className="hero-content">
          <div className="hero-poster">
            {!posterError ? (
              <img src={posterUrl(series.name)} alt={series.name} onError={() => setPosterError(true)} />
            ) : null}
          ...
          <div className="hero-info">
            ...
            {cast.length > 0 && (
              <div className="cast-block">
                ...
                <div className="cast-chips">
                  {cast.map((actor, i) => <span key={i} className="cast-chip">{actor}</span>)}
                </div>
              </div>
            )}
            ...
            <div className="hero-actions">
              {firstVideo && (
                <a href="#" className="btn-primary" onClick={(e) => { e.preventDefault(); playEpisode(firstVideo); }}>
              ...
              >
                ...
                <span className="btn-label-txt">{isNotified ? 'Notifications On' : 'Turn On Notifications'}</span>
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="reveal" style={{ transitionDelay: '.05s' }}>
          {episodes.length === 0 ? (
            <div className="ep-empty">
              ...
              <p className="ep-empty-desc">Episodes will load automatically with a YouTube API Key.</p>
            </div>
          ) : (
            <div className="episodes-grid">
              {episodes.map((ep, i) => (
                <a key={i} href="#" className="ep-card" onClick={(e) => { e.preventDefault(); playEpisode(ep.videoId, i); }}>
          ...
          <>
            ...
            <div className="slider-outer reveal" ref={sliderOuterRef} style={{ transitionDelay: '.15s' }}>
              <div style={{ position: 'relative' }}>
                ...
                <div className="slider-track" ref={sliderTrackRef} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
                  {similar.map((s) => (
                    <a key={s.slug} href="#" className="sim-card" onClick={(e) => { e.preventDefault(); goToSeries(s.slug); }}>
                      <img src={posterUrl(s.name)} alt={s.name} className="sim-img" loading="lazy" />
                      <div className="sim-overlay">
                        ...
                        <div className="sim-meta">
                          ...
                          <span>{s.genre.split(',')[0].trim()}</span>
                        </div>
                      </div>
                    </a>
                  ))}
              ...
              <button className="slider-btn prev" aria-label="Previous" onClick={() => sliderScrollBy('prev')}>
                <svg viewBox="0 0 24 24"><path d="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z" /></svg>
              </button>
              <button className="slider-btn next" aria-label="Next" onClick={() => sliderScrollBy('next')}>
      ...
      <div className="toast-container">
        {toasts.map((msg, i) => <div key={i} className="toast">{msg}</div>)}
      </div>
    </>
  );
}
...
:root {
  --nf-red:#e50914; --nf-red-dim:#b20710;
  --nf-bg:#141414; --nf-bg2:#0a0a0a;
  --nf-surface:#1f1f1f; --nf-surface2:#2a2a2a; --nf-surface3:#333;
  --nf-border:rgba(255,255,255,0.08); --nf-border2:rgba(255,255,255,0.14);
  --nf-text:#e5e5e5; --nf-text2:#a3a3a3; --nf-text3:#6b6b6b;
  --nf-radius:4px;
}
body { font-family:'Outfit',-apple-system,sans-serif; background:var(--nf-bg2); color:var(--nf-text); min-height:100vh; overflow-x:hidden; -webkit-font-smoothing:antialiased; }
...
nav { position:sticky; top:0; z-index:100; background:rgba(10,10,10,0.95); border-bottom:1px solid var(--nf-border); backdrop-filter:blur(20px); -webkit-backdrop-filter:blur(20px); }
.nav-inner { max-width:1280px; margin:0 auto; padding:0 28px; height:64px; display:flex; align-items:center; gap:16px; }
.nav-logo { font-family:'Outfit',sans-serif; font-size:1.5rem; font-weight:900; color:var(--nf-red); letter-spacing:-0.04em; text-decoration:none; flex-shrink:0; }
.nav-search { flex:1; max-width:320px; position:relative; display:flex; align-items:center; }
.nav-search svg { position:absolute; left:12px; width:15px; height:15px; fill:var(--nf-text3); pointer-events:none; }
.nav-search input { width:100%; background:var(--nf-surface); border:1px solid var(--nf-border2); border-radius:3px; padding:8px 14px 8px 36px; font-family:'Outfit',sans-serif; font-size:0.84rem; color:var(--nf-text); outline:none; }
.nav-search input::placeholder { color:var(--nf-text3); }
.nav-back { display:flex; align-items:center; gap:7px; background:none; border:1px solid var(--nf-border2); color:var(--nf-text2); font-size:0.82rem; font-weight:600; padding:8px 16px; border-radius:3px; text-decoration:none; transition:border-color 0.2s,color 0.2s,background 0.2s; white-space:nowrap; }
...
.hero-poster img { width:100%; aspect-ratio:2/3; object-fit:cover; border-radius:3px; border:1px solid var(--nf-border2); box-shadow:0 20px 60px rgba(0,0,0,0.7); }
.poster-ph { width:100%; aspect-ratio:2/3; border-radius:3px; border:1px solid var(--nf-border2); background:var(--nf-surface); display:flex; align-items:center; justify-content:center; font-size:44px; }
...
.badge-genre { background:rgba(255,255,255,0.06); border:1px solid rgba(255,255,255,0.12); color:var(--nf-text2); }
.badge-year { background:rgba(255,255,255,0.06); border:1px solid rgba(255,255,255,0.12); color:var(--nf-text2); }
.badge-demo { background:rgba(255,168,0,0.12); border:1px solid rgba(255,168,0,0.28); color:#ffb347; }
.hero-title { font-family:'Outfit',sans-serif; font-size:clamp(26px,4.5vw,56px); font-weight:900; color:#fff; letter-spacing:-0.03em; line-height:1.05; margin-bottom:14px; }
.hero-desc { font-size:0.88rem; color:var(--nf-text2); line-height:1.75; max-width:520px; margin-bottom:20px; }
.hero-stats { display:flex; gap:28px; flex-wrap:wrap; margin-bottom:24px; }
.h-stat-val { font-family:'Outfit',sans-serif; font-size:26px; font-weight:900; color:var(--nf-red); display:block; line-height:1; margin-bottom:3px; }
.h-stat-lbl { font-size:0.68rem; color:var(--nf-text3); text-transform:uppercase; letter-spacing:0.1em; }
.cast-block { margin-bottom:22px; }
.cast-lbl { font-size:0.68rem; font-weight:700; text-transform:uppercase; letter-spacing:0.1em; color:var(--nf-text3); margin-bottom:7px; }
.cast-chips { display:flex; flex-wrap:wrap; gap:5px; }
.cast-chip { background:rgba(255,255,255,0.05); border:1px solid var(--nf-border2); border-radius:2px; padding:3px 10px; font-size:0.78rem; color:var(--nf-text2); }
.hero-actions { display:flex; flex-wrap:wrap; gap:10px; align-items:center; }
.btn-primary { display:inline-flex; align-items:center; gap:9px; background:#fff; color:#000; font-family:'Outfit',sans-serif; font-size:0.95rem; font-weight:700; padding:12px 26px; border-radius:3px; text-decoration:none; border:none; cursor:pointer; transition:background 0.2s,transform 0.15s; }
.btn-primary:hover { background:rgba(255,255,255,0.85); transform:scale(1.02); }
.btn-primary svg { width:18px; height:18px; fill:#000; }
.btn-secondary { display:inline-flex; align-items:center; gap:9px; background:rgba(109,109,110,0.5); color:#fff; font-family:'Outfit',sans-serif; font-size:0.95rem; font-weight:600; padding:12px 22px; border-radius:3px; border:none; cursor:pointer; transition:background 0.2s; user-select:none; white-space:nowrap; }
...
.sec-label { display:flex; align-items:center; gap:14px; margin-bottom:22px; margin-top:52px; opacity:0; transform:translateY(12px); transition:opacity 0.45s ease,transform 0.45s ease; }
.sec-label.visible { opacity:1; transform:none; }
.sec-label-text { font-size:1.2rem; font-weight:800; color:#fff; letter-spacing:-0.01em; white-space:nowrap; }
.sec-label::after { content:''; flex:1; height:1px; background:var(--nf-border); }
.sec-label-count { font-size:0.78rem; color:var(--nf-text3); font-weight:600; }
...
.ep-card { background:var(--nf-surface); border:none; overflow:hidden; text-decoration:none; display:block; transition:transform 0.2s,z-index 0s; position:relative; z-index:0; }
.ep-card:hover { transform:scale(1.04); z-index:5; box-shadow:0 8px 32px rgba(0,0,0,0.7); }
.ep-thumb { position:relative; overflow:hidden; aspect-ratio:16/9; background:var(--nf-surface2); }
...
.ep-thumb-ph { width:100%; height:100%; display:flex; align-items:center; justify-content:center; background:var(--nf-surface2); color:var(--nf-text3); font-size:28px; }
...
.ep-num-badge { position:absolute; top:8px; left:8px; background:rgba(0,0,0,0.8); border-radius:2px; padding:2px 7px; font-size:0.68rem; font-weight:700; color:var(--nf-text2); }
.ep-info { padding:10px 12px 14px; }
.ep-num { font-size:0.68rem; font-weight:700; text-transform:uppercase; letter-spacing:0.08em; color:var(--nf-red); margin-bottom:3px; }
.ep-title { font-size:0.83rem; font-weight:600; color:var(--nf-text); line-height:1.4; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden; }
.ep-empty { text-align:center; padding:80px 24px; background:var(--nf-surface); border-radius:3px; }
...
.slider-track { display:flex; gap:4px; overflow-x:auto; scroll-snap-type:x mandatory; scrollbar-width:none; -ms-overflow-style:none; padding-bottom:4px; }
...
.slider-btn { position:absolute; top:50%; transform:translateY(-50%); z-index:3; width:40px; height:80px; border:none; background:rgba(30,30,30,0.9); color:#fff; display:flex; align-items:center; justify-content:center; cursor:pointer; transition:background 0.2s; }
.slider-btn:hover { background:rgba(50,50,50,0.95); }
.slider-btn.prev { left:0; }
.slider-btn.next { right:0; }
.slider-btn svg { width:18px; height:18px; fill:currentColor; }
.sim-card { scroll-snap-align:start; flex:0 0 170px; text-decoration:none; display:block; position:relative; overflow:hidden; transition:transform 0.2s,z-index 0s; z-index:0; }
...
footer { position:relative; z-index:1; border-top:1px solid var(--nf-border); padding:28px; max-width:1280px; margin:0 auto; }
...
.footer-logo { font-family:'Outfit',sans-serif; font-size:1.3rem; font-weight:900; color:var(--nf-red); margin-bottom:8px; display:inline-block; }
