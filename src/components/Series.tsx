...
function lsGet(k: string): string[] { try { return JSON.parse(localStorage.getItem(k)||'[]'); } catch { return []; } }
function lsSet(k: string, v: string, on: boolean) { const a=lsGet(k),i=a.indexOf(v); if(on&&i===-1){a.push(v);localStorage.setItem(k,JSON.stringify(a));}else if(!on&&i!==-1){a.splice(i,1);localStorage.setItem(k,JSON.stringify(a));} }
...
export default function SeriesPage() {
  ...
  useEffect(()=>{ if(typeof window!=='undefined'){const p=new URLSearchParams(window.location.search);setUrlSlug(p.get('slug')||'');} },[]);
  ...
  useEffect(()=>{
    ...
    fetchYouTubePlaylist(series.playlist_id)
      .then((eps)=>{ setEpisodes(eps); if(eps.length===0) setEpError('Bu playlist boş veya herkese açık değil.'); })
      .catch((e)=>setEpError(`Hata: ${e.message}`))
  ...
  useEffect(()=>{ if(!slug)return; setIsLiked(lsGet(LS_KEYS.like).includes(slug)); setIsNotified(lsGet(LS_KEYS.notify).includes(slug)); setInWatchlist(isInWatchlist(undefined,slug)); },[slug,isInWatchlist]);
  useEffect(()=>{ if(!series)return; recordWatched({slug:series.slug,name:series.name,poster_url:series.poster_url,rating:series.rating,year:series.year,seasons:series.seasons,genre:series.genre,url:`series?slug=${encodeURIComponent(series.slug)}`,watchedAt:Date.now()}); },[series,recordWatched]);
  useEffect(()=>{ const c=revealRef.current; if(!c)return; const io=new IntersectionObserver((es)=>es.forEach((e)=>{if(e.isIntersecting){e.target.classList.add('visible');io.unobserve(e.target);}}),{threshold:0.08}); c.querySelectorAll('.reveal,.sec-label').forEach((r)=>io.observe(r)); return()=>io.disconnect(); },[series,episodes]);

  const toast=(msg:string)=>{ setToasts((p)=>[...p,msg]); setTimeout(()=>setToasts((p)=>p.slice(1)),2800); };

  // ── NAVİGASYON: store + router.push (İKİSİ BİRDEN)
  const playEpisode=(videoId:string,epIdx:number)=>{
    navigate('player',{slug,video:videoId,ep:String(epIdx)});
    router.push(`/player?slug=${encodeURIComponent(slug)}&video=${encodeURIComponent(videoId)}&ep=${epIdx}`);
  };
  const goHome=()=>{ navigate('home'); router.push('/'); };
  const goSeries=(s:string)=>{ navigate('series',{slug:s}); router.push(`/series?slug=${encodeURIComponent(s)}`); };
  ...
  if(!series) return (
    <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',minHeight:'100vh',color:'#e5e5e5',fontFamily:"'Outfit',sans-serif",background:'#0a0a0a'}}>
      <style>{CSS}</style>
      <h1 style={{fontSize:'2rem',marginBottom:'1rem',color:'#fff'}}>Series bulunamadı</h1>
      <button onClick={goHome} style={{background:'#fff',color:'#000',fontFamily:"'Outfit',sans-serif",fontSize:'0.95rem',fontWeight:700,padding:'12px 26px',borderRadius:'3px',border:'none',cursor:'pointer'}}>← Ana Sayfa</button>
    </div>
  );
    ...
    <>
      ...
      <nav id="nav"><div className="nav-inner">
        ...
        <form className="nav-search" role="search" onSubmit={(e)=>e.preventDefault()}>
          ...
          <input type="text" placeholder="Ara..." autoComplete="off" aria-label="Search" readOnly/>
        </form>
        <a href="/" className="nav-back" onClick={(e)=>{e.preventDefault();goHome();}}>
      ...
      <div className="series-hero">
        ...
        <div className="hero-vignette"/><div className="hero-side-vignette"/>
        <div className="hero-content">
          <div className="hero-poster">
            {!posterErr&&<img src={series.poster_url||posterUrl(series.name)} alt={series.name} onError={()=>setPosterErr(true)}/>}
            <div className="poster-ph" style={{display:posterErr?'flex':'none'}}>🎬</div>
          </div>
          <div className="hero-info">
            <div className="hero-badge-row">
              {series.rating&&<span className="badge badge-rating"><svg viewBox="0 0 24 24" style={{width:10,height:10,fill:'currentColor'}}><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/></svg>{series.rating}</span>}
            ...
            <div className="hero-stats">
              {series.seasons&&<div className="h-stat"><span className="h-stat-val">{series.seasons}</span><div className="h-stat-lbl">Sezon</div></div>}
              {(series as any).episodes&&<div className="h-stat"><span className="h-stat-val">{(series as any).episodes}</span><div className="h-stat-lbl">Bölüm</div></div>}
              {episodes.length>0&&<div className="h-stat"><span className="h-stat-val">{episodes.length}</span><div className="h-stat-lbl">Mevcut</div></div>}
            </div>
            {cast.length>0&&<div className="cast-block"><div className="cast-lbl">Oyuncular</div><div className="cast-chips">{cast.map((a:string,i:number)=><span key={i} className="cast-chip">{a}</span>)}</div></div>}
            <div className="hero-actions">
              {episodes[0]&&(
                <a href={`/player?slug=${encodeURIComponent(slug)}&video=${encodeURIComponent(episodes[0].videoId)}&ep=0`} className="btn-primary"
                  onClick={(e)=>{e.preventDefault();playEpisode(episodes[0].videoId,0);}}>
                  <svg viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
                  <span className="btn-label-txt">İzlemeye Başla</span>
                </a>
              )}
              <button className={`btn-secondary${isLiked?' active':''}`} onClick={()=>{const n=!isLiked;setIsLiked(n);lsSet(LS_KEYS.like,slug,n);toast(n?'Beğenildi!':'Beğeni kaldırıldı.');}}>
                <svg viewBox="0 0 24 24" style={{width:18,height:18}} fill={isLiked?'currentColor':'none'} stroke="currentColor" strokeWidth={2}><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
                <span className="btn-label-txt">{isLiked?'Beğenildi':'Beğen'}</span>
              </button>
              <button className={`btn-secondary${inWatchlist?' active':''}`} onClick={()=>{if(!series)return;const added=toggleWatchlist({slug:series.slug,name:series.name,poster_url:series.poster_url,rating:series.rating,year:series.year,seasons:series.seasons,genre:series.genre,url:`series?slug=${encodeURIComponent(series.slug)}`});setInWatchlist(added);toast(added?'Listeye Eklendi!':'Listeden Çıkarıldı.');}}>
                <svg viewBox="0 0 24 24" style={{width:18,height:18}} fill="none" stroke="currentColor" strokeWidth={2}>{inWatchlist?<polyline points="20 6 9 17 4 12"/>:<><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></>}</svg>
                <span className="btn-label-txt">{inWatchlist?'Listede':'Listeye Ekle'}</span>
              </button>
              <button className={`btn-secondary${isNotified?' active':''}`} onClick={()=>{const n=!isNotified;setIsNotified(n);lsSet(LS_KEYS.notify,slug,n);toast(n?'Bildirimler açık!':'Bildirimler kapalı.');}}>
                <svg viewBox="0 0 24 24" style={{width:18,height:18}} fill="none" stroke="currentColor" strokeWidth={2}><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>
                <span className="btn-label-txt">{isNotified?'Bildirim Açık':'Bildir'}</span>
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="page-body" ref={revealRef}>
        <div className="sec-label reveal">
          <span className="sec-label-text">Bölümler</span>
          {episodes.length>0&&<span className="sec-label-count">{episodes.length} video</span>}
        </div>
        <div className="reveal" style={{transitionDelay:'.05s'}}>
          {epLoading&&<div className="ep-empty"><div className="ep-loading-dots"><span/><span/><span/></div><div className="ep-empty-title" style={{marginTop:16}}>Bölümler yükleniyor…</div></div>}
          {!epLoading&&epError&&<div className="ep-empty"><div className="ep-empty-icon">⚠️</div><div className="ep-empty-title">Bölümler yüklenemedi</div><p className="ep-empty-desc">{epError}</p></div>}
          {!epLoading&&!epError&&episodes.length===0&&<div className="ep-empty"><div className="ep-empty-icon">🎬</div><div className="ep-empty-title">Video bulunamadı</div><p className="ep-empty-desc">Playlist herkese açık olmayabilir veya boş.</p></div>}
          {!epLoading&&episodes.length>0&&(
            <div className="episodes-grid">
              {episodes.map((ep,i)=>(
                <a key={ep.videoId+i}
                  href={`/player?slug=${encodeURIComponent(slug)}&video=${encodeURIComponent(ep.videoId)}&ep=${i}`}
                  className="ep-card"
                  onClick={(e)=>{e.preventDefault();playEpisode(ep.videoId,i);}}>
                  <div className="ep-thumb">
                    ...
                    <div className="ep-overlay"><div className="ep-play-btn"><svg viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg></div></div>
        ...
        {similar.length>0&&<>
          <div className="sec-label reveal" style={{transitionDelay:'.1s'}}><span className="sec-label-text">Benzer Seriler</span></div>
          <div className="slider-outer reveal" style={{transitionDelay:'.15s'}}>
            <div style={{position:'relative'}}>
              <div className="slider-fade-l"/><div className="slider-fade-r"/>
              <div className="slider-track" ref={sliderRef}
                onTouchStart={(e)=>{touchX.current=e.touches[0].clientX;}}
                onTouchEnd={(e)=>{const d=touchX.current-e.changedTouches[0].clientX;if(Math.abs(d)>40)sliderRef.current?.scrollBy({left:d*1.5,behavior:'smooth'});}}>
                {similar.map((s)=>(
                  <a key={s.slug} href={`/series?slug=${encodeURIComponent(s.slug)}`} className="sim-card"
                    onClick={(e)=>{e.preventDefault();goSeries(s.slug);}}>
            ...
            <button className="slider-btn prev" onClick={()=>sliderRef.current?.scrollBy({left:-540,behavior:'smooth'})}><svg viewBox="0 0 24 24"><path d="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z"/></svg></button>
            <button className="slider-btn next" onClick={()=>sliderRef.current?.scrollBy({left:540,behavior:'smooth'})}><svg viewBox="0 0 24 24"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg></button>
          </div>
        </>}
      </div>
      ...
      <footer><div className="footer-logo">{SITE_NAME}</div>
        <p>&copy; {yr} {SITE_NAME} &nbsp;|&nbsp;
          ...
          &nbsp;|&nbsp;<a href="mailto:tvfiremax@gmail.com">İletişim</a>
        </p>
      </footer>
...
::-webkit-scrollbar{width:4px;height:4px}::-webkit-scrollbar-track{background:transparent}::-webkit-scrollbar-thumb{background:#333;border-radius:4px}::-webkit-scrollbar-thumb:hover{background:var(--red)}
...
.cast-block{margin-bottom:22px}.cast-lbl{font-size:0.68rem;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:var(--text3);margin-bottom:7px}
...
.btn-secondary:hover{background:rgba(109,109,110,0.7)}.btn-secondary.active{background:rgba(229,9,20,0.3)}
...
.sec-label.visible{opacity:1;transform:none}.sec-label-text{font-size:1.2rem;font-weight:800;color:#fff;white-space:nowrap}
.sec-label::after{content:'';flex:1;height:1px;background:var(--border)}.sec-label-count{font-size:0.78rem;color:var(--text3);font-weight:600}
.reveal{opacity:0;transform:translateY(16px);transition:opacity 0.5s,transform 0.5s}.reveal.visible{opacity:1;transform:none}
...
.ep-loading-dots span:nth-child(1){animation-delay:-0.32s}.ep-loading-dots span:nth-child(2){animation-delay:-0.16s}
...
.ep-card:hover .ep-play-btn{transform:scale(1)}.ep-play-btn svg{width:20px;height:20px;fill:#000;margin-left:3px}
...
.ep-info{padding:10px 12px 14px}.ep-num{font-size:0.68rem;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:var(--red);margin-bottom:3px}
...
.ep-empty-icon{font-size:40px;margin-bottom:16px}.ep-empty-title{font-size:1.1rem;font-weight:700;color:#fff;margin-bottom:8px}.ep-empty-desc{font-size:0.84rem;color:var(--text2)}
...
.slider-fade-l{left:0;background:linear-gradient(to right,#0a0a0a,transparent)}.slider-fade-r{right:0;background:linear-gradient(to left,#0a0a0a,transparent)}
...
.slider-btn:hover{background:rgba(50,50,50,0.95)}.slider-btn.prev{left:0}.slider-btn.next{right:0}.slider-btn svg{width:18px;height:18px;fill:currentColor}
...
.sim-play svg{width:13px;height:13px;fill:#000;margin-left:2px}.sim-card:hover .sim-play{opacity:1;transform:scale(1)}
...
.sim-meta{display:flex;justify-content:center;gap:7px;font-size:0.67rem;color:var(--text2)}.sim-rating{color:#f0b429}
footer{border-top:1px solid var(--border);padding:28px;max-width:1280px;margin:0 auto}
footer p{font-size:0.8rem;color:var(--text3)}footer a{color:var(--text3);text-decoration:none;transition:color 0.2s}footer a:hover{color:var(--text2)}
...
@media(max-width:768px){.hero-content{grid-template-columns:1fr;gap:20px;padding:70px 20px 40px}.hero-poster{width:110px;margin:0 auto}.hero-info{text-align:center}.hero-badge-row,.hero-stats,.cast-chips,.hero-actions{justify-content:center}.hero-desc{margin-left:auto;margin-right:auto}.page-body{padding:0 16px 60px}.sim-card{flex:0 0 130px}.nav-search{display:none}}
@media(max-width:480px){.hero-actions .btn-label-txt{display:none}.btn-primary{padding:11px 20px;font-size:0.88rem}}
`;
